  ...
  return (
    <div className="flex flex-col gap-6">
      <p className="-mt-4 text-sm text-muted-foreground">
        {data.period} · {data.rows.length} chatters closing
      </p>
      <StatKpis kpis={data.kpis} />
      {data.rows.length === 0 ? (
        <div className="rounded-lg border border-dashed p-8 text-center">
          <p className="text-sm font-medium">Aucun chatter closing à classer</p>
          <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
            Aucun chatter n&apos;est désigné setter/closer ET lié à un membre sur cette période.
            Désigne le closing sur la fiche Membre, puis lie le membre à son chatter (⚠️ dans la
            liste Membres).
          </p>
        </div>
      ) : (
