 ...
 * Mois`, `PRIME TOP15 SETTER` et `PRIME TOP3 MOIS`, une ligne par chatteur.
 *
 * ── POURQUOI ICI, DANS L'ONGLET CLASSEMENT, ET PAS DANS UN QUATRIÈME ONGLET ──────────────────
 * Trois raisons, dans l'ordre où elles comptent :
 *  1. Deux des quatre colonnes SONT le sujet de cet onglet — la prime setter est la conclusion du
 ...
 *     notion de « mois d'une période » pouvant contredire celle qui garde l'argent (index 0092).
 ...
 *
 * ⚠️ DEUX GRAINS, ET L'ÉCRAN LE DIT. `CA` et `Handoffs` sont des totaux du MOIS CIVIL ; les deux
 * primes sont des montants de PÉRIODE, sommés sur les 2 ou 3 périodes rattachées au mois. Les
...
export function ComptaMonthView({ data }: { data: MoisData }) {
  ...
  const totalMonthly = data.rows.reduce((s, r) => s + r.monthlyPrime, 0)
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p className="text-sm text-muted-foreground">
        ...
        {frDayShort(data.month.end)}. Les primes, elles, se versent par période : ce sont celles
        des {data.periods.length} périodes rattachées à ce mois —{' '}
      ...
      {data.rows.length === 0 ? (
        ...
        <div className="grid grid-cols-[1fr_auto_auto_auto_auto] items-center gap-x-6 gap-y-1.5 text-sm">
          ...
          <span className={`${COL_HEAD} text-right`}>Prime du mois</span>
          ...
          <span className="border-t pt-1.5 text-right font-medium tabular-nums">{eur2(totalMonthly)}</span>
        </div>
      )}
 ...
 *
 * La PRIME DU MOIS porte la période qui la saisit, en dessous. Sans elle, un montant mensuel
 * affiché dans un tableau mensuel n'a aucun point de saisie identifiable : le lecteur ne sait pas
 * sur laquelle des 2 ou 3 fiches de la période il doit aller la corriger.
 */
function MonthLine({ row }: { row: MoisRow }) {
  ...
  return (
    <>
      ...
      <span className={row.monthlyPrime === 0 ? muted : 'text-right tabular-nums'}>
        {eur2(row.monthlyPrime)}
        {row.monthlyPrimePeriod && (
          <span className="block text-[10px] text-muted-foreground">{row.monthlyPrimePeriod}</span>
        )}
      </span>
    </>
  )
