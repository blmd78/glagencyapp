...
export async function recordPayment({
  ...
  const { error } = await supabase.from('compta_payments').insert({
    ...
    // La prime setter (colonne 0091) est FIGÉE ici pour la même raison que `ca_reference` : sa
    // source bouge après coup. Elle se RECALCULE à chaque rendu depuis les handoffs saisis — un
    // handoff corrigé la semaine suivante changerait le rang, donc la prime, d'un membre déjà
    // payé. Aucune valeur par défaut sur cette colonne : l'omettre ne compilerait pas (spec
    // §5.3). Ses voisines `carryover_amount` et `monthly_prime_amount` ont été droppées par la
    // migration 0095 avec leurs concepts (décision de Benoit, 2026-07-28).
