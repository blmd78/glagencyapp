...
/**
 * Formulaire de la config du test (admin), en deux onglets dans UN seul <form> (même pattern que
 * le dialog des modules, RHF garde les valeurs des onglets non montés) : « Config » (ouverture,
 * déroulé, seuils, lien Discord) et « Test » (le contenu des épreuves : texte de frappe + banque
 * QI). Un onglet dont un champ est en erreur porte un « • » rouge. Une seule ligne en base, un
 * seul bouton : tout part ensemble (`saveRecruitConfig`). Les seuils sont les GATES CACHÉS du
 * verdict — le candidat ne les voit jamais, ni avant ni après.
 *
...
export function ConfigForm({ config }: { config: RecruitConfigData }) {
  ...
  const configBad = !!(
    errors.botMessages || errors.qiTimer || errors.qiMin || errors.frappeMin ||
    errors.connexionMin || errors.globalThreshold || errors.discordLink
  )
  const testBad = !!(errors.typingText || errors.qiBank)
    ...
    <form onSubmit={(e) => void submit(e)} className="flex max-w-3xl flex-col gap-6">
      <div className="flex items-start gap-3">
        <Controller
          control={control}
          name="open"
          render={({ field }) => <Switch id="open" checked={field.value} onCheckedChange={field.onChange} disabled={isSubmitting} />}
        />
        <span className="grid gap-1">
          <Label htmlFor="open" className="text-sm font-medium">Test ouvert</Label>
          <span className="text-xs text-muted-foreground">
            Fermé, la page /postuler affiche « Le recrutement est fermé pour le moment » — effet immédiat, y compris sur les tests en cours.
          </span>
        </span>
      </div>
      ...
      <Tabs defaultValue="config" className="flex flex-col gap-6">
        <TabsList className="self-start">
          <TabsTrigger value="config">Config{dot(configBad)}</TabsTrigger>
          <TabsTrigger value="test">Test{dot(testBad)}</TabsTrigger>
        </TabsList>
