...
/**
 * « Intégrer » : crée le compte du candidat, en UN clic. Depuis la liste ou depuis sa fiche.
 *
 * PLUS DE CHOIX DE MODÈLE (0147). Le dialog repris de Good Luck Agency (`index.html:2313-2326`)
 * demandait la modèle au moment de l'intégration ; elle n'est presque jamais décidée à cette
 * seconde-là, et la sortie « Créer le compte sans rattacher » était devenue le geste courant. Elle
 * est maintenant le geste UNIQUE : la personne entre au CRM **en formation** (`in_training`), et
 * son rattachement se fait plus tard depuis Membres ou le board Organisation — qui posent tous
 * deux `integrated_at` et éteignent `in_training` (trigger 0147).
 *
    ...
    handler: async ({ id }): Promise<{ profileId: string }> => {
      ...
      const { data: patched, error: pErr } = await admin
        ...
        .update({
          ...
          // EN FORMATION (0147) : c'est l'état de quelqu'un qui sort du test de recrutement — il
          // n'a pas encore de modèle, par construction. Avec `is_new`, ce sont les DEUX drapeaux
          // cochés à l'arrivée.
          in_training: true,
          // `integrated_at` (0129) n'est PAS posé ici : il n'y a plus de rattachement à
          // l'intégration. `syncAssignments` (members/authz.ts:100-107) le posera au premier, ce
          // qui est exactement son rôle — la date dit l'entrée en PRODUCTION, pas l'entrée au CRM.
          //
