...
import type { DayReportRow } from '../types'
  ...
  return (
    <details className="group border-b last:border-b-0">
      <summary className="grid cursor-pointer list-none grid-cols-[0.5rem_1fr_1rem] items-center gap-3 px-4 py-2.5 hover:bg-muted/40 sm:grid-cols-[0.5rem_minmax(8rem,1fr)_minmax(12rem,2fr)_5rem_5rem_1rem] [&::-webkit-details-marker]:hidden">
        {/* Pastille d'état — le `span.dot` de l'ancien board. ROUGE seulement sur le créneau
            ATTENDU : ailleurs c'est du renfort, et une ligne de renfort a par construction une
            couverture minuscule (16 % de moyenne en production). La colorer en rouge
            reviendrait à sanctionner le zèle — c'est la décision D7 de la spec. */}
        <span
          aria-hidden
          className={cn(
            'size-2 rounded-full',
            !row.isExpected ? 'bg-muted-foreground/40' : row.held ? 'bg-emerald-500' : 'bg-red-500',
          )}
        />
