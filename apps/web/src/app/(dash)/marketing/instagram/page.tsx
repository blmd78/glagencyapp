...
export default async function MktInstagramPage({
  ...
  const data = getMktSocial('instagram', period)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        <MktInstagramContent data={data} links={links} />
      </Suspense>
    </div>
  )
}

async function MktInstagramContent({
  data,
  links,
}: {
  ...
  return <MktSocialTemplate data={d} links={l.filter((x) => x.type === 'instagram')} />
}
